...
GT911_Status_t GT911_ReadTouch(TouchCoordinate_t *coordinate, uint8_t *number_of_coordinate) {
	...
	if ((StatusRegister & 0x80) != 0) {
		*number_of_coordinate = StatusRegister & 0x0F;
		//printf("%d\n", *number_of_coordinate);
		if (*number_of_coordinate != 0) {
			for (uint8_t i = 0; i < *number_of_coordinate; i++) {
				...
				coordinate[i].x = RxBuffer[0];
				coordinate[i].x = (RxBuffer[1] << 8) + coordinate[i].x;
				coordinate[i].y = RxBuffer[2];
				coordinate[i].y = (RxBuffer[3] << 8) + coordinate[i].y;
